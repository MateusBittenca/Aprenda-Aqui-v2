...
export function UnitRowActions({
  ...
  async function reorder(direction: "up" | "down") {
    setLoading(true);
    try {
      await fetch(`/api/professor/units/${unitId}/reorder`, {
        method: "PATCH",
  ...
  async function handleDelete() {
    if (!window.confirm(`Excluir a unidade "${title}" e todas as lições?`)) return;
    setLoading(true);
    try {
      const res = await fetch(`/api/professor/units/${unitId}`, { method: "DELETE" });
      if (!res.ok) {
  ...
  return (
    <div className="flex flex-wrap gap-2 shrink-0">
      ...
      <Button asChild variant="secondary" size="sm" className="rounded-xl font-bold">
        <Link href={`/professor/unidades/${unitId}/editar`}>
